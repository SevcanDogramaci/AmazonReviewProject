...
def perform_db_scan_and_print(data, original_data, review_bodys, min_samples_val, eps_val, dataset_name):
    ...
    dbscan = DbScan()
    dbres = dbscan.perform_db_scan(data, min_samples_val, eps_val)  # inside dbscan.py
    
    plotter = Plotter()
    title = dataset_name + " - eps: " + str(eps_val) + ", min_samples: " + str(min_samples_val)
...
review_bodys = list(data_frame["review_body"])
...
review_bodys = preproc.clear_reviews(original_review_bodys)
...
perform_db_scan_and_print(tf_idf_review_bodys, original_review_bodys, review_bodys, 2, 1.0, dataset_name)
